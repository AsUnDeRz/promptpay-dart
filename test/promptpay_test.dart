...
void main() {
  ...
  test('generate qr data without amount', () {
    expect(PromptPay.generateQRData("0000000000"),
  ...
  test('should generate promptpay data for phone number correctly', () {
    expect(PromptPay.generateQRData("0812345678", amount: 123.45),
        "00020101021129370016A000000677010111011300668123456785802TH53037645406123.4563043045");
  });
  test('should generate promptpay data for identity correctly', () {
    expect(PromptPay.generateQRData("1100599000373", amount: 123.45),
        "00020101021129370016A000000677010111021300000000000005802TH53037645406123.4563046DCC");
  });

  test('should generate promptpay data for e-wallet correctly', () {
    expect(PromptPay.generateQRData("000000000000000", amount: 123.45),
        "00020101021129370016A00000067701011103150000000000000005802TH53037645406123.456304AE16");
  });
  ...
  test('extract qrcode from qr data', () {
    ...
    final accountTypeString =
        qrData.substring(indexOfStartAccountType, indexOfStartAccountType + 2);

    expect(accountTypeString, "01");

    final accountType = accountTypeString == "01"
        ? AccountType.phone
        : accountTypeString == "02"
    ...
    final accountLength =
        int.parse(qrData.substring(indexOfStartAccountLength, indexOfStartAccountLength + 2));

    expect(accountLength, 13);

    final indexOfStartAccountData = indexOfStartAccountLength + 2;
    final accountData =
        qrData.substring(indexOfStartAccountData, indexOfStartAccountData + accountLength);
